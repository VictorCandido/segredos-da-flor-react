...
export default async function handler (req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const buildResponse: any = {
            'GET': async () => responseMessage = await CustomerController.list(req),
            'PUT': async () => responseMessage = await CustomerController.update(req),
            'DELETE': async () => responseMessage = await CustomerController.delete(req),
